...
所有源文件都是 IIFE,挂载到 `window.aeditor`:
```js
;(function (aeditor) {
  'use strict'
  // ...
  aeditor.something = something
...
aeditor/
  ...
  src/
    core/                          # ⚠ 原 src/core/ 已并入这里(重构后的现状)
      ...
      errors.js                    # aeditor.errors signal + reportError + safeCall + 全局 window 兜底
...
- 写 dev server 时用 `.claude/launch.json` 已配好的 `aeditor-demo`(端口 5570),不要自己拉新端口
...
- 错误系统 aeditor.errors / reportError / safeCall + window error/unhandledrejection 全局兜底(§ 4.7)
...
**主题系统**:
- `src/style/theme.css` 三层 token:Layer 1 原子(`--aeditor-c-00..11` + 4 个语义色)→ Layer 2 角色(`--aeditor-bg-N` / `--aeditor-fg-N` / `--aeditor-border*` / `--aeditor-accent*`) → Layer 3 组件(`--aeditor-toolbar-h` 之类极少数特例)
- 派生半透明色统一用 `color-mix(in srgb, var(--aeditor-c-XX) NN%, transparent)`
- **三套内置主题**,通过 `:root[data-aeditor-theme=...]` 切换(缺省属性 = dark):
  - **dark(默认)** —— Godot Minimal 风:`#272727` 中性炭灰 ramp,`#569eff` 冷蓝 accent,"inset 输入框"角色映射(`--aeditor-bg-2` 比 `--aeditor-bg-1` 深)
  - **dracula** —— 冷调深灰 + `#7b6ef6` 紫 accent,"raised 输入框"映射(`--aeditor-bg-2` 比 `--aeditor-bg-1` 亮) + 更强阴影
...
- Tooltip / popover:`aeditor-fade-in` / `aeditor-pop-in` 入场动画
...
2. **`aeditor.effect(() => ...)` 也是同步触发**。如果 effect 体里向 `documentElement.style` 写 inline CSS variable,初次挂载那一刻就会把当前 signal 值写成 inline 样式,**inline specificity 会覆盖 `[data-aeditor-theme="light"]` 之类的属性选择器,主题切换从此失效**。修复模板:在 effect 里读 `getComputedStyle` 的 effective value,和想写的 literal 比较,相同就 return 跳过写入 —— 初次挂载零污染,只有用户真的编辑才写 inline。详见 `demo/components/theme-config.js` 的 `bindWriter`。
...
- **绝不绑任何快捷键**(参见 § 2.1)
- **已知限制**:当 aeditor root 元素的某个祖先设置了 CSS `transform` / `filter` / `perspective` / `will-change: transform` 时,`position:fixed` 会相对该祖先建立包含块而不是视口,focus 模式的 dock 不会铺满屏幕。这是 CSS 规范行为,框架不兜底;需要时调用方自己把 aeditor root 挂到 `<body>` 直接子级或用 `<dialog>` portal
...
### 4.7 错误处理系统
统一的错误处理 + panel 错误隔离:
- `aeditor.errors`:`signal([])`,每条 `{ id, time, source: { scope, dockId?, panelId?, component? }, error, message, stack }`
- `aeditor.reportError(source, err)` / `aeditor.clearErrors()` / `aeditor.dismissError(id)`
- `aeditor.safeCall(source, fn)`:try/catch 包裹,失败 push 到 errors 并返回 `null`
- 所有 component `factory` / `dispose` 的调用都走 `safeCall` 包裹(同步边界)
- 单个 panel 出错只显示红色错误框,**不影响其他 panel**
- 内置 component `error-log`:订阅 `aeditor.errors` 渲染错误列表,用户可以把它放进任何 dock 当 "Problems" 面板用

**异步错误兜底**:`safeCall` 只抓同步调用栈,component 内部 `setTimeout` / `Promise` / `addEventListener` 抛的错抓不到。框架入口(`createDockLayout` 首次调用时)注册一次性的全局监听:
- `window.addEventListener('error', e => aeditor.reportError({ scope: 'global' }, e.error))`
- `window.addEventListener('unhandledrejection', e => aeditor.reportError({ scope: 'global' }, e.reason))`
...
**`signal.js` 的 effect cleanup 错误走 `console.error`,不路由 `aeditor.errors`**。这是架构分层必然的裁决:`errors.js` 用 `signal([])` 定义 `aeditor.errors`,因此依赖 `signal.js`;反过来若 `signal.js` 调 `aeditor.reportError`,就成了循环依赖。边界划清 —— **signal.js 是零依赖底层,它的错误路径只走 `console.error`**。语义上也是对的:effect cleanup 失败是框架底层契约破裂(component 的 `onCleanup` 回调崩了),属于"fail-loud 到控制台"的范畴,不是归到某个 panel 的软错误列表里可以慢慢看的事。
...
```js
aeditor.registerComponent(name, {
...
**配套 API**:
- `aeditor.registerComponent(name, spec)` —— 注册,重名 throw,名字必须是合法 string
- `aeditor.resolveComponent(name)` —— 查表返回 spec,未注册 throw
- `aeditor.componentDefaults(name)` —— `resolveComponent(name).defaults?.() ?? {}`,调用点不用判空
...
- **PanelData**(内容单元):
  ...
  ```
  **`props` 的硬性约束**(和 § 4.8 同一条):不许塞函数 / DOM / class instance / Map / Set / 循环引用。想传"行为"走 `aeditor.bus`,不要塞进 props。框架不做运行时校验 —— 这是调用方契约(§ 2.5)。
...
- **DockRuntime**(每个 tree 里的 dock 一份):
  ```
  {
    ...
    dockEl:                HTMLElement,                   // .aeditor-dock 根(keyed reconcile 的锚点)
    toolbarEl:             HTMLElement | null,            // .aeditor-toolbar(无 toolbar 时为 null)
    contentEl:             HTMLElement,                   // .aeditor-dock-content(active panel 的唯一挂载点)
...
ctx.bus                  // auto-unsub 版 aeditor.bus,on() 自动挂当前 runtime 的 cleanups
...
**id 生成永远走全局计数器**:`aeditor._nextPanelId()` / `aeditor._nextDockId()` / `aeditor._nextToolbarItemId()`,这三个计数器挂在 `aeditor` 上,进程内单调,跨 LayoutRuntime 共享。用户不传 id、不看 id(除非从返回值里读)—— 这是 § 4.11 的硬规矩。
...
### 4.13 通讯总线 aeditor.bus(pub/sub,自动清理)
...
- **每个 handler 都被 `safeCall` 单独包裹**:某个 handler 抛错只会路由到 `aeditor.errors`(source 带订阅者所在 panel 的 dockId/panelId),**不会中断同一 emit 的其他订阅者**。这是 § 4.7 错误隔离规则的延伸 —— 一条总线上互不信任的 component 需要有这个保证
- **auto-unsubscribe**:component 通过 `ctx.bus.on(topic, h)` 订阅时,框架自动把退订函数塞进 panel runtime 的 `cleanups` 列表,panel dispose 时一起清,**不存在订阅泄漏**。component 直接用 `aeditor.bus.on` 就没有自动清理,需要自己管
- 框架内置几个系统 topic(component 可订阅,也可不订阅):
  - `aeditor:panel:activated` → `{ dockId, panelId }`
  - `aeditor:panel:removed` → `{ dockId, panelId }`
  - `aeditor:panel:moved` → `{ panelId, fromDockId, toDockId }`(跨 dock / 跨窗口都发,跨窗口时 fromDockId 是对端 id,by 约定 prefix 区分)
  - `aeditor:dock:focus-changed` → `{ dockId, focused }`
  - `aeditor:errors:new` → 新增 `aeditor.errors` 条目时(`error-log` component 其实就订阅这个)
...
- "panel 已激活" 通知 → **bus**(`aeditor:panel:activated`),因为这是事件,错过了不需要补
- "文件内容已修改"(dirty 标记) → **signal**(`ctx.panel.dirty`),状态
- "文件已保存" → **bus**(`'file:saved'` 之类),事件
- 跨 panel 共享的"当前选中的对象" → **signal**(由某个权威 panel 持有,放在 `aeditor` 上或通过 bus 广播一次拿到 ref;真要解耦还是首选"权威 panel emit 事件,关心方各自维护本地 cache")
- 一次性命令(关闭 / 保存 / 编译) → **bus**
...
- 新老窗口通过 `BroadcastChannel('aeditor-layout')` 建立连接
...
- **CSS 变量前缀 `--aeditor-*`**:所有颜色、边距、圆角、阴影、字号、时长都走 `var(--aeditor-*, fallback)` 形式,第一行就开始,不要硬编码散落。这不是主题系统(框架不提供运行时 `setTheme()`),只是命名规范。默认值对齐 `doc/editor_style.html` 的调色板。分类约定:
  - `--aeditor-color-bg-*` / `--aeditor-color-fg-*` / `--aeditor-color-border` / `--aeditor-color-accent` / `--aeditor-color-hover` / `--aeditor-color-active`
  - `--aeditor-space-1..5`(4 / 8 / 12 / 16 / 24)
  - `--aeditor-radius-sm / --aeditor-radius-md`
  - `--aeditor-shadow-sm / --aeditor-shadow-md / --aeditor-shadow-lg`
...
- **DOM 卫生**:
  ...
  - 所有可交互组件统一四态,由 CSS 变量供色:`hover` → `--aeditor-color-hover`,`active` → `--aeditor-color-active`,`focus` → `outline: 2px solid var(--aeditor-color-accent)`,`disabled` → `opacity: 0.38; pointer-events: none`
...
- **拖放视觉反馈**(§ 4.14 的视觉层规约):
  ...
  - 合法 drop zone:`--aeditor-color-accent` 半透明蒙层;非法(被 `accept` 白名单拒):红色 1px 描边 + 禁入光标
  - 分割线 hover:`--aeditor-color-accent` 高亮;拖动时全局 cursor 锁定
...
1. **Phase 1 — 核心层(零 tree / 零 UI 改动)** ✅
   ...
   - `core/errors.js`:全局错误系统 —— `reportError` / `safeCall` / `aeditor.errors` signal(**此时不挂 window 监听**,留到 layout 入口)
...
5. **Phase 4 — 错误隔离 + bus 通讯** ✅
   - `components/error-log.js`:订阅 `aeditor.errors` 的列表 component,同时订阅 `aeditor:errors:new` bus topic 以便测试 bus 路径(两条数据源等价)
...
**下一步给下个 Claude 的提示**:
- 进项目第一件事:`node tools/build.mjs --watch` + `.claude/launch.json` 的 `aeditor-demo`(端口 5570)
