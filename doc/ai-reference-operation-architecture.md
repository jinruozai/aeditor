...
EditorFrame treats AI as another editor operator. The framework should not know
...
```js
EF.ai.operations.register("gde.patch", {
...
```js
EF.ai.attach(rowEl, GDE.refs.entity(table, id), { drag: true })
EF.ai.attach(labelEl, GDE.refs.field(table, id, field), { menu: true })
```

`EF.ai.attach` is an alias for the existing target binding behavior. The visible
