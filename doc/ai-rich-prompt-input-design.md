...
Scope: EditorFrame framework-level rich prompt input for AI chat/send panels
...
EditorFrame uses a **plain string with private token characters plus a sidecar token map**.
...
`resourceId` points to an entry in `EF.ai` resource storage. The token is only the inline reference. The resource pool owns payload resolution, deduplication, permissions, and persistence.
...
No business code should mutate `text` and `tokens` separately. All mutations go through `EF.ui.richPromptInput` helpers.
...
```html
<span
  class="ef-richprompt-token"
  data-ef-token="..."
  data-ef-resource-id="res_icon"
  contenteditable="false"
  role="button"
  aria-label="Reference icon.png"
>
  <span class="ef-richprompt-token-label">icon.png</span>
  <button class="ef-richprompt-token-remove" type="button" tabindex="-1">x</button>
</span>
```
...
1. `application/x-ef-rich-prompt`: restore draft fragment and referenced resources.
...
1. `application/x-ef-rich-prompt`: preserves token refs inside EditorFrame.
...
```js
EF.ai.richPrompt.empty()
EF.ai.richPrompt.normalize(draft)
EF.ai.richPrompt.allocateToken(draft)
EF.ai.richPrompt.insertText(draft, index, text)
EF.ai.richPrompt.insertRef(draft, index, resource)
EF.ai.richPrompt.deleteRange(draft, start, end)
EF.ai.richPrompt.refs(draft)
EF.ai.richPrompt.toPlainText(draft)
EF.ai.richPrompt.toModelText(draft)
EF.ai.richPrompt.toClipboard(draft)
EF.ai.richPrompt.fromClipboard(data)
```
...
Delta drawbacks for EditorFrame prompt input:
...
EditorFrame can still export/import a Delta-like shape later if needed. Internally, the PUA token string is smaller and more stable for this exact use case.
...
4. Replace `ai-chatinput` textarea with `EF.ui.richPromptInput`.
