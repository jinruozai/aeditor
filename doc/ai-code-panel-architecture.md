...
```txt
AI-created visible UI uses editor.createPanel.
editor.createPanel accepts one factory function source.
The extension manifest remains an internal implementation detail.
```

AI should not normally call `editor.installExtension` for new UI. That tool is
still available for advanced full-extension work, but the stable path for new
panels is `editor.createPanel`.
...
JavaScript panel code and uses the same `EF.ui.*`, `ctx`, `EF.bus`, and DOM APIs
...
`EF.ui.renderUITree` does not execute such templates, so the UI can install
...
2. **One AI entry point for visible UI**
   AI uses `editor.createPanel` for a new visible panel. It should not handcraft
...
editor.createPanel({
  ...
  source: `
    function (propsSig, ctx) {
      ...
      const title = EF.ui.text({ value: 'Inventory', variant: 'h2' })
...
```js
EF.registerComponent('inventory-panel', {
  title: 'Inventory',
  icon: 'package',
  defaults: function () {
...
```txt
Use editor.createPanel for new UI panels.
...
- Use EF.ui.* where useful; prefer framework components over hand-built
  controls when the library already has a suitable component.
- Use EF.ui.scrollArea for scrollable content instead of raw native overflow
  scrollbars so the panel matches EditorFrame styling.
- Use EF.ui.tooltip/popover/menu for anchored floating UI. Scoped EF.ui overlays
  ...
  `EF.ui.registerScopedOverlay(anchor, close)`.
...
- Do not call EF.registerComponent; the runtime registers the component.
- Do not mutate unrelated editor state except through approved tools or ctx APIs.
- Clean timers/listeners with EF.ui.collect(root, cleanup) or ctx.onCleanup when available.
```
...
`editor.createPanel` again with the same stable id.

## Operation Semantics

`editor.createPanel` is both an operation and a tool.
...
The model can repair by calling `editor.createPanel` again with the same id.
...
Internally, `editor.createPanel` sets:

```js
allowCode: true
```

and routes through the same permission checks as `editor.installExtension`.
...
5. The source does not call registration or extension lifecycle APIs directly:
   - `EF.registerComponent`
   - `EF.unregisterComponent`
   - `EF.extensions.install`
   - `EF.extensions.uninstall`
...
```js
const maker = Function('EF', '"use strict"; return (' + source + ')')(EF)
...
not be the primary authoring model because it makes using `EF.ui.*` and `ctx`
less direct.
...
- Async factories are not supported in the primary path. If async data is
  ...
  it later.
- The factory should not call `EF.registerComponent`; the runtime has already
...
function (propsSig, ctx) {
  ...
  if (EF.ui && EF.ui.collect) EF.ui.collect(root, function () { clearInterval(timer) })
  return root
}
```
...
Generated docs and prompt examples should teach `EF.ui.collect(root, cleanup)`
...
function (propsSig, ctx) {
  ...
  if (EF.ui && EF.ui.collect) {
    EF.ui.collect(root, function () {
...
The runtime default dispose calls `EF.ui.dispose(root)`, so collected cleanups
...
`editor.createPanel` result should include:
...
AI should prefer `editor.createPanel` over `editor.installExtension` for new UI.
`editor.installExtension` remains the low-level escape hatch for full extension
...
- AI tool registration: `editor.createPanel`
- AI operation registration: `editor.createPanel`
...
editor.createPanel for new UI panels".
...
- direct `EF.registerComponent` in source rejected
...
Repair:
Call editor.createPanel again with the same id.
Replace template strings with normal JavaScript loops and textContent.
```

## Why This Is The Best Fit

This design keeps editorframe simple:
...
- One AI UI tool: `editor.createPanel`.
...
- Existing `EF.ui.*` components remain available.
