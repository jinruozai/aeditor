...
This plan keeps EditorFrame aligned with the product principles: simple, elegant, efficient, visually consistent, and cleanly separated from project code.
...
- No browser-native `prompt/confirm/alert` in editor workflows; use `EF.ui.prompt`, `EF.ui.confirm`, or callback-provided dialogs.
- All user-visible text in GDE goes through `t()` / `I18N`.
- User-facing strings must be valid UTF-8 and free of mojibake.
- CSS color must use `--ef-*` semantic tokens or project-level `--gde-*` tokens derived from them.
...
- `npm run check:dist`
- `npm run check:gde`
- Rebuild `dist/ef.js` and `dist/ef.css`.
- Sync `temp/GameDataEditor/vendor/ef.js` and `ef.css`.
