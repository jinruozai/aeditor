...
- AI-created visible UI should use `editor.createPanel`, which wraps a same-page `factory(propsSig, ctx)` panel into this runtime.
- `editor.installExtension` and `editor.addPanelToDock` remain low-level/advanced operations, not the normal AI UI authoring surface.

Implemented Phase 1:

- Owner-aware component / reference / operation registration and owner cleanup.
- `EF.extensions.preview(...)`, `install(...)`, `uninstall(...)`, `list(...)`, `get(...)`.
- `EF.extensions.update(...)`, `enable(...)`, `disable(...)`, `boot(...)`, `safeMode(...)`.
- `EF.extensions.setMaxLayer(...)`, `disableLayer(...)`, `enableLayer(...)` for recovery filtering.
- `EF.extensions.configureStorage(...)`, `save(...)`, `clearStored(...)` for manifest persistence.
- `EF.extensions.review(...)`, `installWithReview(...)`, and `configurePermissions(...)` for install/update permission review.
- `EF.commands` owner-aware command registry and menu seam registry.
- Extension-owned settings sections / schemas / pages with owner cleanup.
- Declarative panel contributions backed by existing `EF.registerComponent(...)` and `EF.ui.renderUITree(...)`.
...
- Layout registration bridge through `EF.extensions.registerLayout(...)`; `createDockLayout(...)` registers itself as `default` unless given `config.name`.
- AI operations: `editor.installExtension`, `editor.updateExtension`, `editor.removeExtension`, `editor.enableExtension`, `editor.disableExtension`, `editor.promoteExtensionLayer`, `editor.addPanelToDock`, `editor.removePanelFromDock`.
- Trusted code panel contributions are supported only when install/update passes explicit `allowCode: true`; AI-created visible UI now defaults to `editor.createPanel`.
...
This document defines the proposed runtime extension architecture for editorframe. It captures the dynamic panel discussion and folds it into the existing Dock / Panel / Component and AI Reference / Operation model.

The goal is not to add a one-off "dynamic panel" feature. The goal is to let an editor built on editorframe grow new capabilities at runtime while keeping the framework simple, inspectable, reversible, and consistent with the current component model.

## Core Idea

editorframe already has the right foundation:
...
1. **No second component world**
   Extensions install into the existing registries. A generated panel, a project panel, and a built-in panel are all `EF.registerComponent(...)` entries.

2. **Manifest lifecycle**
   Extension capabilities are represented internally as serializable manifests. A manifest can be previewed, diffed, approved, persisted, promoted, reverted, and disabled. AI visible UI does not handcraft these manifests; `editor.createPanel` creates them internally.
...
5. **AI uses Operations**
   AI does not mutate extension runtime directly. For visible UI it calls `editor.createPanel`; advanced extension lifecycle still flows through registered operations such as `editor.installExtension`, `editor.removeExtension`, and `editor.addPanelToDock`, all under the normal permission system.

6. **Factory panel is the AI default**
   AI-created visible panels should be same-page `factory(propsSig, ctx)` panels created through `editor.createPanel`. Declarative UI trees remain a secondary static format.
...
The scheme is aligned with editorframe's philosophy because it keeps the original dependency direction:
...
The design is intentionally not "perfect" in the sense of allowing arbitrary customization. It is perfect for editorframe only if it stays narrow:
...
editorframe should keep its own style: zero build, IIFE, explicit registration, no hidden mutation observers, and no broad "wrap any function" hook system.
...
|---|---|---:|---|
| `core` | Framework built-ins | No | editorframe |
...
```js
EF.extensions.setMaxLayer('core')
EF.extensions.boot({ safeMode: true })
EF.extensions.safeMode(true, { allowApp: true })
EF.extensions.disableLayer('user')
```
...
```js
EF.registerComponent('my-panel', spec)
```

Owner is required for extension-managed registration, but extension authors should not write it manually. `EF.extensions.install(...)` injects it.

```js
EF.registerComponent('inventory-panel', spec, {
  owner: 'extension:project.characterInventory',
  layer: 'project',
})
```
...
```js
EF.unregisterComponentOwner('extension:project.characterInventory')
EF.ai.operations.unregisterOwner('extension:project.characterInventory')
EF.ai.references.unregisterOwner('extension:project.characterInventory')
// Later phase, after EF.commands exists:
EF.commands.unregisterOwner('extension:project.characterInventory')
```
...
```js
EF.registerComponent('project.characterInventory/panel', spec, { owner, layer })
```
...
editorframe exposes a small `EF.commands` registry. It deliberately does not bind global shortcuts or application behavior; it only stores commands, runs explicit command ids, and provides menu seam items for host UI to render.
...
```js
EF.commands.menuUiItems('dock.panel.context', ctx)
```
...
```js
EF.extensions.registerAdapter('project.readInventory', {
...
```js
EF.extensions.install(manifest, options)
EF.extensions.uninstall(id, options)
EF.extensions.get(id)
EF.extensions.list(filter)
EF.extensions.preview(manifest)
EF.extensions.registerLayout(name, handle)
EF.extensions.registerAdapter(id, spec)
EF.extensions.update(id, nextManifest, options)
EF.extensions.enable(id)
EF.extensions.disable(id)
EF.extensions.boot(options)
EF.extensions.safeMode(enable)
EF.extensions.configureStorage(options)
EF.extensions.setMaxLayer(layer)
EF.extensions.disableLayer(layer)
EF.extensions.enableLayer(layer)
```

Registry owner support:

```js
EF.registerComponent(name, spec, { owner, layer })
EF.unregisterComponent(name, { owner })
EF.listComponents({ owner, layer })
EF.unregisterComponentOwner(owner)
EF.ai.operations.unregisterOwner(owner)
EF.ai.references.unregisterOwner(owner)
```
...
editor.removeExtension
editor.enableExtension
editor.disableExtension
editor.promoteExtensionLayer
editor.addPanelToDock
editor.removePanelFromDock
...
```txt
Built-in UI component      -> EF.registerComponent
Built-in panel             -> EF.registerComponent
App panel                  -> EF.registerComponent
Extension UI component     -> EF.extensions.install -> EF.registerComponent
Extension panel            -> EF.extensions.install -> EF.registerComponent
```

Rendering can reuse `EF.ui.renderUITree` or a small wrapper around it.
...
- Code panels must use `ctx.onCleanup` and `EF.ui.dispose`.
...
```js
{
  ...
  hash: EF.extensions.hashSource(srcdoc)
}
```
...
```js
const review = EF.extensions.review(manifest, { actor, agentId })
// review.canInstall, review.canApply, review.permissions, review.requiredConsent
```

Hosts may use `EF.extensions.installWithReview(...)` for a small built-in confirmation helper, or render their own review UI from the same object. Permission policy can be customized with:
...
`EF.extensions.preview(...)` and the preview phase of `editor.installExtension` should return:
...
```js
EF.extensions.configureStorage({
  key: 'editorframe.extensions.v1',
...
If the host creates dock layout before extensions are installed, panels that reference extension components will fail. Therefore extension boot can register extension components before layout creation, while dock panel contributions may be deferred. `EF.extensions.registerLayout(...)` applies pending dock panel contributions when the named layout becomes available. Explicit layout names never fall back to another layout.
...
No AI-facing `editor.createPanel` yet in this phase; code panels arrive through the later trusted factory path.
...
- no replacing `EF.registerComponent`
- no broad low-code language beyond simple UI tree/actions
- no implicit command/menu hooks outside the explicit `EF.commands` registry
...
1. Calls `editor.getCapabilities` / layout read tool.
2. Generates same-page panel factory source.
3. Calls `editor.createPanel` with `layer: 'session'`.
...
AI creates visible panels through editor.createPanel and uses low-level extension operations only for advanced extension work.
```

This is compatible with editorframe's original philosophy and makes the editor grow without turning the framework into a pile of special cases.
