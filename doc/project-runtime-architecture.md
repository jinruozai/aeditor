# EditorFrame Project Runtime Architecture

This document defines the final architecture for EditorFrame as a local-first,
AI-editable editor runtime.

The goal is not to add another parallel plugin system. The goal is to make every
editor built on EditorFrame follow one simple shape:
...
1. One programming model.
   ...
   the same `EF.registerComponent` / `factory(propsSig, ctx)` component model.
...
3. Dynamic panels remain useful as drafts.
   `editor.createPanel` is still the fast path for one-shot UI generation, but
...
4. Open directory equals open editor.
   ...
   project directory and EditorFrame can mount the corresponding app.
...
EditorFrame has three layers:

```text
Host Shell
  owns global EF runtime, project picker, permissions, recovery UI

Project Runtime
  opens a workspace directory, loads editorframe.project.json,
...
User selects directory
  -> EF.workspace opens an authorized root
  -> EF.project reads editorframe.project.json
  -> project entry registers panels/tools/resources
  -> EF.createDockLayout mounts layout into the host dock
...
`editorframe.project.json` is the single discovery file.
...
| Field | Meaning |
| --- | --- |
| `type` | Must be `editorframe-project`. |
...
with `setup` / `mount` methods, or call `EF.project.define(...)` while loading.
...
`EF.project.define(spec)` is the canonical project registration entry point.
...
function InventoryPanel(propsSig, ctx) {
  const root = EF.ui.h('div', 'inventory-panel')
...
- Use `EF.ui.*` components when available.
- Floating overlays should use `EF.ui.tooltip`, `EF.ui.popover`, `EF.ui.menu`,
  or `EF.ui.registerScopedOverlay`.
- Panel root layout must be responsive inside a resizable dock.

## 6. Workspace Runtime

`EF.workspace` is the file access boundary.
...
| `events` | `ctx.bus`, `EF.bus`, DOM events, cleanup handlers. |
...
`editor.createPanel` remains as the fast drafting path.

```text
User asks for quick UI
  -> AI calls editor.createPanel
...
The project runtime converts this into the existing `EF.dock`, `EF.panel`, and
`EF.split` tree shape. The Dock runtime remains the only layout engine.
...
that origin. The workspace API bounds EditorFrame and AI file access, but it does
...
4. Keep AI file operations inside `EF.workspace` even when project code is trusted.
...
6. Panel factory failure shows a panel error box and records `EF.log`.
...
EditorFrame's zero-build same-page runtime uses snapshot rollback today because
...
- Panels already use `EF.registerComponent`.
...
the EditorFrame host can mount the same app into a dock.
...
- Uses EditorFrame panels and bus topics.
...
`editor.createPanel` can continue to wrap a factory source as a session extension.
Promotion converts that session extension into project files.

## 15. Minimal Public API

```js
EF.project.open(workspace, options)
EF.project.close(id)
EF.project.reload(id)
EF.project.current()
EF.project.list()

EF.project.define(spec)       // called by project entry
EF.project.promotePanel(input)

EF.workspace.openDirectory(options)
EF.workspace.fromHandle(handle)
EF.workspace.fromBridge(root)
EF.workspace.memory(files)
```

The host usually needs only:

```js
const workspace = await EF.workspace.openDirectory()
const project = await EF.project.open(workspace, { mount: container })
```
...
- Convert `editor.createPanel` session panels into project panel files.
- Update `layout.json`.
- Reload project.
