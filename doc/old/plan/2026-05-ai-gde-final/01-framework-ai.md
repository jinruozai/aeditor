...
aiditor.ai.reparentAgent(id, parentAgentId, order)
aiditor.ai.moveAgent(id, patch)
aiditor.ai.selectAgent(id)

aiditor.ai.agent.send(agentId, message, actor)
aiditor.ai.agent.read(agentId, actor)
aiditor.ai.agent.messages(agentId, opts, actor)
...
aiditor.ai.registerConnection(id, connection)
aiditor.ai.registerAuthDriver(type, driver)
aiditor.ai.registerTransport(type, driver)
aiditor.ai.registerTool(id, tool)
aiditor.ai.registerSkill(id, skill)
aiditor.ai.registerTargetProvider(id, provider)
aiditor.ai.bindTarget(el, targetOrFn, opts)
...
aiditor.ai.rejectToolCall(agentId, callId, actor)
