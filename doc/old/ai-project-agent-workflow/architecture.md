...
7. Writes return the new hash. Undo journal support is planned, not part of the
   current workspace adapter contract.
...
`ProjectMap` is the planned repo map adapted to editor projects.
...
```text
readFileRange/readSource -> patchFile(baseHash, patches) -> runCheck -> inspectPanel
```
...
```text
project.createFile src/panels/foo.panel.js
project.patchFile layout.json
project.reload
project.inspectPanel
```
...
- Keep base-hash write checks. Add undo journal as a later hardening item.
...
- Add richer `project.describe` and project map indexing on top of current
  `project.searchFiles` and `project.readSource`.
