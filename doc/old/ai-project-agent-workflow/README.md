...
This directory contains the research and final architecture for Aiditor as a
project-aware AI coding editor.

The central decision:

> Aiditor should treat AI coding as file-backed project work, not as long
...
- [architecture.md](architecture.md) - final Aiditor architecture proposal.

## Final Direction

Aiditor keeps the current Dock / Panel / Component model as the UI runtime, and
...
1. One UI programming model: built-in panels, project panels, and AI-created
   panels use the same `aiditor.registerComponent` and `factory(propsSig, ctx)` shape.
...
- `doc/project-runtime-architecture.md` defines opening a directory as an
  Aiditor project. This directory extends that idea with the agent workflow
