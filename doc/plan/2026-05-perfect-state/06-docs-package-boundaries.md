...
- `AGENTS.md` still contains stale references to `src/core/errors.js`, `EF.errors`, and `error-log`.
...
- Replace stale `EF.errors` references with `EF.log`, or explicitly decide to add compatibility aliases.
...
- `index.html` loads `dist/ef.js` first
- `demo/` consumes `EF`
