...
- EF.ui cleanup lives on `el.__efCleanups` and requires `EF.ui.dispose(el)`.
...
- removes `ef-root` from the container if no longer used
...
- If no `spec.dispose` exists and `EF.ui.dispose` exists, call `EF.ui.dispose(contentEl)`.
- If neither applies, remove the node.

This makes built-in and user components safer without forcing every user panel to remember `ctx.onCleanup(() => EF.ui.dispose(root))`.
...
- Calling `EF.ui.dispose(contentEl)` by default could surprise user components that already remove children in custom `dispose`.
