...
EF.ui components use a local cleanup model:

```js
el.__efCleanups = [fn, fn, ...]
EF.ui.dispose(el)
```
...
- Some helpers create `EF.effect()` without `ui.collect()`.
- Some composite components append child EF.ui components without disposing them.
- Virtualized rows are removed instead of disposed.
- Overlay content is unmounted but not always disposed.
- Registered panel components return EF.ui components but do not expose `dispose`.

## Target rule

Every EF.ui component must obey this invariant:

> If an EF.ui component creates an effect, derived signal, DOM event listener, timer, overlay, portal, or child EF.ui component, disposing the component root with `EF.ui.dispose(root)` must release it.
...
- Running a repeated create/destroy smoke with representative EF.ui components leaves no active component effects.
...
- Registered built-in panel components can rely on runtime default `EF.ui.dispose` fallback or have explicit dispose definitions.
