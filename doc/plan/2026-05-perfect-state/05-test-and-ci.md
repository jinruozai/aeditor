...
- running build would produce the committed `dist/ef.js` and `dist/ef.css`
...
These can run in Node with a minimal `global.window = { EF: {} }` shim or by evaluating IIFE sources in order.
