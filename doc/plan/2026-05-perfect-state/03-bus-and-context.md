...
- event-style communication goes through `EF.bus`
...
- Documentation still references `EF.errors` while implementation uses `EF.log`.
...
```js
EF.bus.on(topic, handler)
EF.bus.off(topic, handler)
EF.bus.emit(topic, payload)
```

Raw `EF.bus` stays simple and has no owner context.
...
1. Keep `EF.bus` unchanged. `ctx.bus.on` wraps the handler with `ctx.safeCall`.
2. Extend `EF.bus.on(topic, handler, source)` internally but keep public docs focused on the two-argument form.
...
- Keep `EF.bus` as user/app communication infrastructure.
...
- `EF.log.dismiss(id)`
- `EF.log.clear()`
- `EF.reportError(source, err)`
- `EF.safeCall(source, fn)`
...
- Keep `EF.log` as the more general primitive.
- Document `EF.reportError` and `EF.safeCall` as error-specific helpers.
- If compatibility aliases are desired, add them deliberately:
  - `EF.errors = EF.derived(() => EF.log().filter(e => e.level === 'error'))`
  - `EF.clearErrors()` removes only error entries or clears all? This ambiguity makes alias risky.

Recommendation: update docs to `EF.log` instead of adding ambiguous aliases.
