...
EditorFrame keeps browser AI integration split into three layers:
...
|---|---:|---|
| `EF_AI_BRIDGE_HOST` | `127.0.0.1` | HTTP bind host. Keep loopback for local auth. |
| `EF_AI_BRIDGE_PORT` | `8787` | HTTP port used by `openai-codex` and local bridge connections. |
| `EF_CODEX_COMMAND` | `codex` | Command that starts the official Codex app server. |
| `EF_CODEX_ARGS` | `app-server --listen stdio://` | Arguments for the Codex app server JSONL transport. |
| `EF_CODEX_CHAT_COMMAND` | empty | Optional override for chat requests. The command receives JSON on stdin and returns JSON or text on stdout. |
| `EF_CODEX_CHAT_ARGS` | empty | Arguments for `EF_CODEX_CHAT_COMMAND`. |
...
| `/connections/openai-codex/chat` | `POST` | Send an EditorFrame AI request through Codex. |
...
- The bridge accepts EditorFrame request JSON, not arbitrary shell commands. `EF_CODEX_CHAT_COMMAND` is a developer override for local testing and controlled integrations.
