...
This document is the implementation target for the EditorFrame AI agent runtime.
...
- `EF.ai.agents` stores agent state.
- `EF.ai.appendMessage` stores messages.
- `EF.ai.runAgent` executes one agent turn.
- `EF.ai.agent.send` exists as a tool-level operation.
...
Use names that match existing EF style while keeping the final model clean.
...
- Implement `EF.ai.message.send`.
- Implement `EF.ai.message.read`.
