// aeditor.ui.enumInput — bitmask flags editor (toggle multiple flags from a set).
//
// opts:
...
;(function (aeditor) {
  ...
  ui.enumInput = function (opts) {
    ...
    const el = ui.h('div', 'aeditor-ui-enum')
    const items = o.options || []
    const btns = []
    for (let i = 0; i < items.length; i++) {
      const it = items[i]
      const b = ui.h('button', 'aeditor-ui-enum-btn', { type: 'button', text: it.label, title: '0x' + it.value.toString(16) })
    ...
    ui.bind(el, sig, function (v) {
      for (let i = 0; i < btns.length; i++) btns[i].b.classList.toggle('aeditor-ui-enum-on', !!(v & btns[i].val))
    })
    return el
  }
