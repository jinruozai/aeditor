// EF.project - file-backed EditorFrame project runtime.
;(function (EF) {
  'use strict'
  ...
  function define(spec) {
    if (!spec || !spec.id) throw new Error('EF.project.define: spec.id is required')
  ...
  function normalizeDescriptor(raw) {
    const d = clone(raw || {})
    if (d.type !== 'editorframe-project') throw new Error('Invalid project descriptor type')
  ...
  function cleanupOwner(owner) {
    if (EF.ai && EF.ai.unregisterToolOwner) EF.ai.unregisterToolOwner(owner)
  ...
  function makeContext(runtime) {
    ...
    const ctx = {
      ...
      component: function (id, spec) {
        return EF.registerComponent(id, spec, meta)
      },
      command: function (id, spec) {
        return EF.commands.register(id, spec, meta)
      },
      menu: function (id, spec) {
        return EF.commands.registerMenu(id, spec, meta)
      },
      tool: function (id, spec) {
        if (!EF.ai || !EF.ai.registerTool) throw new Error('Project tool registry is not available')
        requirePermission(runtime, 'ai.tools.register')
        return EF.ai.registerTool(id, spec, meta)
      },
      reference: function (id, spec) {
        if (!EF.ai || !EF.ai.references) throw new Error('Project reference registry is not available')
        return EF.ai.references.register(id, spec, meta)
      },
      operation: function (id, spec) {
        if (!EF.ai || !EF.ai.operations) throw new Error('Project operation registry is not available')
        return EF.ai.operations.register(id, wrapOperation(runtime, spec), meta)
      },
      settingsSection: function (id, spec) {
        return EF.settings.registerSection(id, spec, meta)
      },
      settingsSchema: function (sectionId, schema) {
        return EF.settings.registerSchema(sectionId, schema, meta)
      },
      settingsPage: function (id, spec) {
        return EF.settings.registerPage(id, spec, meta)
      },
      style: function (path) {
      ...
      createDockLayout: function (container, config) {
        ...
        return EF.createDockLayout(container, config)
      },
  ...
  async function installStyle(runtime, path) {
    ...
    el.setAttribute('data-ef-project-style', runtime.id)
  ...
  async function open(workspace, options) {
    options = options || {}
    const descriptor = normalizeDescriptor(await readJson(workspace, 'editorframe.project.json'))
  ...
  async function reload(id, options) {
    ...
    const descriptor = normalizeDescriptor(await readJson(workspaceFor(old), 'editorframe.project.json'))
  ...
  function projectForTool(projectId) {
    ...
    if (!p) throw new Error('No open EditorFrame project')
    return p
  }
  ...
  function findDraftSource(input) {
    ...
    if (!input.extensionId || !EF.extensions || !EF.extensions.get) return ''
    const ext = EF.extensions.get(input.extensionId)
  ...
  function promotedPanelFile(input, componentId, source) {
    ...
    return [
      '// EditorFrame project panel: ' + componentId,
      ';(function (EF) {',
      "  'use strict'",
      '',
      '  EF.registerComponent(' + jsString(componentId) + ', {',
  ...
  async function readSourceProjection(runtime, path, options) {
    ...
    if (projection === 'outline' || projection === 'events') {
      ...
      for (let i = 0; i < lines.length; i++) {
        const line = lines[i]
        if (projection === 'outline' && /^\s*(function|const|let|var)\s+[\w$]+|EF\.registerComponent|EF\.project\.define/.test(line)) {
          out.push({ line: i + 1, text: line.trim() })
        }
        if (projection === 'events' && /ctx\.bus|EF\.bus|addEventListener|removeEventListener|onCleanup|registerScopedOverlay/.test(line)) {
          out.push({ line: i + 1, text: line.trim() })
        }
      }
  ...
  function registerAiTools() {
    ...
    const owner = 'ef.project'
    EF.ai.registerTool('project.readDescriptor', {
      title: 'Read Project Descriptor',
      description: 'Read the current EditorFrame project descriptor.',
    ...
    EF.ai.registerTool('project.searchFiles', {
    ...
    EF.ai.registerTool('project.readFile', {
    ...
    EF.ai.registerTool('project.readFileRange', {
    ...
    EF.ai.registerTool('project.readSource', {
    ...
    EF.ai.registerTool('project.writeFile', {
    ...
    EF.ai.registerTool('project.createFile', {
    ...
    EF.ai.registerTool('project.patchFile', {
    ...
    EF.ai.registerTool('project.deleteFile', {
    ...
    EF.ai.registerTool('project.promotePanel', {
    ...
    EF.ai.registerTool('project.updateDescriptor', {
      title: 'Update Project Descriptor',
      description: 'Update editorframe.project.json in the current project.',
      schema: { type: 'object', required: ['descriptor'], properties: { projectId: { type: 'string' }, descriptor: { type: 'object' }, baseHash: { type: 'string' } } },
      run: function (args) {
        return workspaceFor(projectForTool(args && args.projectId)).write('editorframe.project.json', JSON.stringify(args.descriptor || {}, null, 2), { baseHash: args.baseHash })
      },
    }, { owner: owner, layer: 'builtin' })
    EF.ai.registerTool('project.reload', {
      title: 'Reload Project',
      description: 'Reload the current EditorFrame project after file edits.',
      schema: { type: 'object', properties: { projectId: { type: 'string' } } },
      run: function (args) { return reload(args && args.projectId) },
    }, { owner: owner, layer: 'builtin' })
    EF.ai.registerTool('project.inspectPanel', {
    ...
    EF.ai.registerTool('project.runCheck', {
